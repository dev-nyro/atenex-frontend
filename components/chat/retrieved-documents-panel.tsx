...
import { FileText, Info, Star, Download, Eye, X } from 'lucide-react';
...
export function RetrievedDocumentsPanel({ documents, isLoading }: RetrievedDocumentsPanelProps) {
    ...
    const ScoreStars = ({ score }: { score: number | null | undefined }) => {
      if (score == null || score < 0) return <span className="text-[10px] text-muted-foreground italic">N/A</span>;
                ...
                <div className="p-3 space-y-3">
                        ...
                        <div className='space-y-2'>
                            {[...Array(3)].map((_, i) => (
                                <Card key={`skel-${i}`} className="p-3 border border-border/50 bg-card opacity-70">
                                    <div className="flex justify-between items-center mb-1.5"> <Skeleton className="h-4 w-3/4 rounded" /> <Skeleton className="h-3 w-8 rounded-sm" /> </div>
                                    <Skeleton className="h-3 w-full rounded" /> <Skeleton className="h-3 w-1/2 rounded mt-1" />
                                    <div className="flex justify-between items-center mt-2"> <Skeleton className="h-2.5 w-16 rounded" /> <Skeleton className="h-3 w-3 rounded" /> </div>
                                </Card>
                            ))}
                        </div>
                    )}
                    {!isLoading && documents.length === 0 && (
                        <div className="flex flex-col items-center justify-center text-center text-muted-foreground py-10 px-4 min-h-[200px]">
                            ...
                            <p className="text-sm font-medium mb-1">Sin Fuentes</p> <p className="text-xs">No se encontraron fuentes específicas para la última consulta.</p>
                        </div>
                    )}
                    {documents.map((doc, index) => (
                        <DialogTrigger asChild key={doc.id || `doc-${index}`}>
                            <Card
                                className={cn("cursor-pointer transition-all duration-150 bg-card/95 hover:bg-accent/10", "border border-border/70 hover:border-primary/60 hover:shadow-lg focus-visible:ring-2 focus-visible:ring-primary/40 focus-visible:border-primary/40" )}
                                onClick={() => handleViewDocument(doc)} onKeyDown={(e) => e.key === 'Enter' && handleViewDocument(doc)} tabIndex={0}
                                title={`Ver detalles de: ${doc.file_name || doc.cita_tag || 'documento'}`}
                            >
                                <CardContent className="p-0">
                                    <div className="flex items-center gap-2 px-3 pt-3">
                                        <span className="flex-shrink-0 h-6 w-6 flex items-center justify-center text-xs font-mono bg-muted text-muted-foreground rounded-full border border-border/60">{index + 1}</span>
                                        <div className="flex-1 min-w-0">
                                            <span className="block font-medium text-foreground/95 truncate text-sm" title={doc.file_name || doc.cita_tag || `Fragmento ${doc.id}`}>
                                                {doc.file_name || doc.cita_tag || `Fragmento ${doc.id.substring(0, 8)}...`}
                                            </span>
                                            <span className="block text-[11px] text-muted-foreground truncate">
                                                ID Doc: {doc.document_id && doc.document_id !== doc.id ? `${doc.document_id.substring(0,8)}...` : 'N/D'} / Frag: {doc.id.substring(0, 8)}...
                                            </span>
                                        </div>
            ...
            {selectedDoc && (
                 <DialogContent className="sm:max-w-2xl lg:max-w-3xl grid-rows-[auto_minmax(0,1fr)_auto] max-h-[85vh] p-0">
                    <DialogHeader className="px-6 pt-6 pb-4 border-b">
                        <DialogTitle className="truncate text-lg flex items-center gap-2" title={selectedDoc.file_name || selectedDoc.cita_tag || selectedDoc.document_id}>
                        ...
                        <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-muted-foreground pt-1">
                            {selectedDoc.document_id && selectedDoc.document_id !== selectedDoc.id && (
                                <span>ID Doc: <span className="font-mono text-[11px]">{selectedDoc.document_id}</span></span>
                            )}
                    ...
                    <ScrollArea className="overflow-y-auto px-6 py-4 max-h-[calc(85vh-200px)] sm:max-h-[calc(85vh-180px)]"> {/* Ajuste de altura máxima */}
                         <div className="space-y-4 text-sm">
                            <div>
                                <Label className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Contenido Completo del Fragmento:</Label>
                                <div className="prose prose-sm dark:prose-invert max-w-none break-words prose-p:leading-relaxed prose-ul:my-2 prose-ol:my-2 prose-pre:my-2 prose-blockquote:my-2 mt-1 border-l-4 pl-4 max-h-[300px] overflow-y-auto pretty-scrollbar">
                                    <Markdown remarkPlugins={[remarkGfm]}>
                                        {selectedDoc.content || selectedDoc.content_preview || "Contenido no disponible."}
                                    </Markdown>
                                </div>
                            </div>
